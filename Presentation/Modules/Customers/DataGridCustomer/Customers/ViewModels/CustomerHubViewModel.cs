...
{
    public class CustomerHubDataGridViewModel : BindableBase, INavigationAware
    {
        #region Members
        ...
        #region Constructors
        public CustomerHubDataGridViewModel(IUnityContainer container, IEventAggregator eventAggregator, IDialogViewService dialogViewService, ICustomerService customerService)
        {
        ...
        #region Properties
        public string Title => $"Customers({CustomerList.AllCustomers.Count})";
        ...
        internal async Task CreateCustomerListViewModelAsync()
        {
            ...
            try
            {
                ...
                void AddPropertyChanged()
                {
                    ...
                    {
                        if (sender is CustomerListViewModel clvm)
                        {
                            if (e.PropertyName == nameof(CustomerListViewModel.IsLoading) && !clvm.IsLoading)
                            {
                                IsLoading = false;
                            }
            ...
            {
                errorMessage = $"The following error messages were {nameof(CustomerHubDataGridViewModel)} : { Environment.NewLine}{ex.Message}";
            }
            finally
            {
                if (IsLoading)
